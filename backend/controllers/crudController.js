...
const login = async (req, res, next) => {
  /*
	...
	*/
  const username = req.body.inputs.email; // by right, it is supposed to be username, however state is email instead, will leave for now
  ...
  try {
    if (await bcrypt.compare(password, user.passwordHash)) {
      ...
      const user = { name: username };
      const accessToken = jwt.sign(user, process.env.ACCESS_TOKEN_SECRET);
      use = await User.findOne({ username: username });
      res.send({
