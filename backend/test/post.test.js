...
// 			const response = await request.post("/api/posts").send(POST_A);
// 			expect(response.statusCode).toBe(400);
// 			done();
// 		});
// 		it("create post successfully", async (done) => {
// 			const response = await request.post("/api/forum").send(FORUM_A);
// 			forum = response.body;
...
// 		beforeAll(async (done) => {
// 			await request.post("/api/users/login").send(USER_A_LOGIN);
...
// 			const response = await request.get(`/api/posts/${post._id}`);
...
// 			const response = await request.get(`/api/posts/${INVALID_ID}`);
// 			expect(response.statusCode).toBe(400);
